...
[![Version](https://img.shields.io/badge/version-1.8.10.0-00a4dc)](meta.json)
...
- **Full cast & crew accordion** — Cast, Directing, Writing, Production, Camera, Editing, Sound, Art, Costume & Make-Up, Visual Effects, Lighting, Crew, and Other, with roles collapsed per person (unique names, “+N more”, tooltip). Cards open the Jellyfin person page when that person exists locally.
...
- **Library Stats** — Home header tab next to Favourites → `#/fullcrew/stats` with charts, auto insights, per-category drill-downs, and bucket item lists (e.g. every title that is Stereo / HEVC). Special audio (commentary, descriptions, dubs) is one of those charts.
- **Special audio** — Stats → Special audio lists labeled extra tracks across the library. Item detail pages show chips that jump to the matching Stats bucket.
...
2. Copy `Jellyfin.Plugin.FullCrew.dll` and `meta.json` into a **single** plugin folder, e.g. `plugins/Jellyfin.Plugin.FullCrew_1.8.10.0/`. Remove any older `Jellyfin.Plugin.FullCrew_*` / `Full Crew_*` folders first — Jellyfin 10.11 will try to load every copy.
...
| Include special audio in Stats | on | Special audio chart + `/FullCrew/audio` API |
...
Open a movie, series, season, or episode in Jellyfin Web. Below the usual detail content you’ll get a **Full Cast & Crew** accordion. Clicking a person opens their **Jellyfin** person page when they exist in your library (TMDB links stay on that person page).
...
Open **Stats → Special audio**, or `#/fullcrew/stats/audioTracks`. A type bucket is `#/fullcrew/stats/audioTracks/items/commentary`.

The ranking is commentary, audio description, dub, isolated score/effects, karaoke, and other labeled titles. Ordinary “English / Stereo / AAC” dialogue tracks are ignored. Detail pages show chips that open the matching Stats bucket. Old `#/fullcrew/audio` links redirect here.
