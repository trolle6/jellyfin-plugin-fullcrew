...
public class PluginFolderAccessTests
{
    ...
    {
        var assembly = new Version(1, 8, 10, 0);
        var safe = PluginFolderAccess.StartupSafeVersion(assembly);

        Assert.Equal("1.8.10", safe.ToString());
        Assert.NotEqual(assembly.ToString(), safe.ToString());
        Assert.NotEqual("1.8.10.0", safe.ToString());
    }
    ...
    {
        Assert.Equal("1.8.10", PluginFolderAccess.StartupSafeVersion(null).ToString());
    }
